...
class once extends core{
	...
	function item_delete(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($this->once_delete_item('pages','project_id')){
				...
				if($stmt->rowCount()){
					foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $rowx) {
						...
						if($this->settings['perm_delete']){
							...
							$this->data['source']='';
							$this->data['id']=$this->data['page_id'].'_'.$rowx['id'];
	...
	function item_new(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($this->once_insert_item('pages')){
				// Create empty page file
				@mkdir("../pages/");
				chmod("../pages/", 0777);
				file_put_contents('../pages/page_'.$this->item['id'].'.php','');
			}
		}
		return $this->once_response();
	}
	...
	function item_grid_delete(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($stmt->rowCount()){
				...
				$stmt->bindParam(':page_id', $row['page_id'], PDO::PARAM_INT);
				...
				// Check if setting allow to perm_delete // Refresh styles / scripts and ajax requests
				if($this->settings['perm_delete']){
					@unlink('../pages/page_'.$row['page_id'].'_'.$temp.'.php');
						
					// Make prefix to reconize pages
					$this->data['type']='pages';
					$this->data['path']='css';
					$this->data['file']='style.css';
					$this->data['id']=$row['page_id'].'_'.$temp;
	...
	function item_grid_select(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($stmt->rowCount()){
				...
				if($row['plugin_id']!=$this->data['value'] && $this->data['value']>0){
					// Prepare statements to get all layers
					...
					if($stmt->rowCount()){						
						...
						copy('../once/plugins/'.$this->data['value'].'/plugin.php','../pages/page_'.$row['page_id'].'_'.$row['id'].'.php');
				...
				$this->item['plugins']=$rowx['plugins'];
			...
			if($stmt->rowCount()){
				...
				if($this->data['type']=='plugins-theme'){
					...
					if($this->once_insert_item('plugins')){
						...
						// Copy object files
						copy('../pages/page_'.$row['page_id'].'_'.$row['id'].'.php','./plugins/'.$this->item['id'].'/plugin.php');
					...
					if($this->once_insert('snippets',array(
						...
						// Make snippet dir
						@mkdir("./snippets/".$this->item['id']."");
						@chmod("./snippets/".$this->item['id']."", 0777);

						// Prepare statements to get snippet template.
						$tpl['source']=@file_get_contents($this->data['root_path'].'/once/default/snippet.php');
						
						// Create preview file
						file_put_contents('./snippets/'.$this->item['id'].'/index.php',$tpl['source']);

						// Copy object files
						copy('../pages/page_'.$row['page_id'].'_'.$row['id'].'.php','./snippets/'.$this->item['id'].'/snippet.php');
						...
						$this->data['file']='style.css';
						$this->data['id']=$row['page_id'].'_'.$row['id'];
						file_put_contents('./snippets/'.$this->item['id'].'/snippet.css',$this->once_read_source());
							
						$this->data['type']='pages';
						$this->data['path']='js';
						$this->data['file']='script.js';
						$this->data['id']=$row['page_id'].'_'.$row['id'];
						file_put_contents('./snippets/'.$this->item['id'].'/snippet.js',$this->once_read_source());
					}
				}
			}
		}
 	...
 	function item_grid_visibility(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($stmt->rowCount()){
				...
				$temp=$this->data['id'];
				$this->data['id']=$row['page_id'].'_'.$this->data['id'];
 	...
 	function load_source(){
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($this->data['file']=='head.php'){
				...
				$this->item['source']=@file_get_contents($this->data['root_path'].'/pages/page_'.$this->data['page_id'].'_'.$this->data['id'].'.php');
			}else{
		...
		if($this->once_csrf_token_check($this->data['csrf_token']) && $this->once_creator_check()){
			...
			if($stmt->rowCount()){
				...
				if($this->data['file']=='head.php'){
					...
					$this->item['source']=@file_put_contents($this->data['root_path'].'/pages/page_'.$this->data['page_id'].'_'.$this->data['id'].'.php',$this->data['source']);
					$this->item['source']=$this->gen_page($this->data['page_id']);
				}else{
