...
if($_GET['option']!=''){
	...
	$stmt = $once->pdo->prepare("SELECT * FROM edit_themes_categories WHERE LOWER(name) LIKE :category");
...
# GET DATA -------------------
$types=$once->category_get('themes');
...
# GET DATA -------------------
$data=$once->once_select_items_page('themes');
?>
<div id="themes-data" class="box" data-ajax="true" data-c="<?php echo $_GET['c'];?>" data-o="<?php echo $_GET['o'];?>" data-type_id="<?php echo $_GET['type_id'];?>" data-category_id="<?php echo $once->url_slug($_GET['option']);?>" data-sort_by="<?php echo $_GET['sort_by'];?>" data-ids="<?php echo $_GET['idsxs'];?>" data-page="<?php echo $_GET['page'];?>" data-query="<?php echo $_GET['query'];?>">
	...
	<div class="row box-body">
		...
		if(isset($data['items'])){
			...
			foreach($data['items'] as $key => $val){
				if($data['items'][$key]['id']==$_SESSION['project_id']){
					$active='
					<div class="col-md-4" id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
						<div class="theme-item theme-active">
							<div class="row">
								<div class="col-md-12">
									<div class="theme-thumb">
										...
										<img src="themes/'.$data['items'][$key]['id'].'/thumbnail.png?'.time().'" onerror="this.src=\'/once/img/theme.png\'">
									</div>
								</div>
							</div>
							<div class="row theme-footer">
								<div class="col-md-6 theme-name">
									'.$data['items'][$key]['name'].'  
								</div>
								<div class="col-md-6">
									<div class="btn-group" role="group">
										<button type="button" class="btn btn-default btn-sm pull-right item-edit"><i class="fa fa-check-square-o"></i>&nbsp;&nbsp;&nbsp; Current</button>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="col-md-4" id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
						<div class="theme-item">
							...
							<div class="row theme-footer">
								<div class="col-md-4 theme-name">
									'.$data['items'][$key]['name'].'  
								</div>
								<div class="col-md-8">
									<div class="btn-group pull-right" role="group">
										<button type="button" class="btn btn-success btn-sm item-use"><i class="fa fa-check"></i>&nbsp;&nbsp;&nbsp; Use</button>
										<button type="button" class="btn btn-default btn-sm item-edit"><i class="fa fa-edit"></i></button>
									</div>
								</div>
							</div>
						</div>
			...
			echo '
			<div class="col-md-12">
				Not found any themes here, be first and create it once!
			</div>';
		}
		?>
	</div>
			...
			if(isset($data['page'])){
				...
				<ul class="pagination">';
					for($i=1;$i<=$data['pages'];$i++){
						echo '
						<li><a href="/themes'.$urla.'/page/'.$i.''.$urlb.'" '.($_GET['page']==$i?'class="active"':'').'>'.$i.'</a></li>';
					}
					echo '
				</ul>';
			}
...
<script type="text/javascript">
	$(document).ready(function () {
		// Initialize actions
		once.themes.actions.mainInit($(this));
	});
</script>