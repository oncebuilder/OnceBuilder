...
# DECLARE SORT ARRAY -------------------
$data_sort=array('','id DESC','id ASC','time DESC','time ASC','name DESC','name ASC');
...
if($_GET['option']!=''){
	...
	$stmt = $once->pdo->prepare("SELECT * FROM edit_mailbox_types WHERE LOWER(name) LIKE :category");
...
<div id="mailboxs-data" class="box" data-ajax="true" data-c="<?php echo $_GET['c'];?>" data-o="<?php echo $_GET['o'];?>" data-type_id="<?php echo $_GET['type_id'];?>" data-category_id="<?php echo $once->url_slug($_GET['option']);?>" data-sort_by="<?php echo $_GET['sort_by'];?>" data-ids="<?php echo $_GET['idsxs'];?>" data-page="<?php echo $_GET['page'];?>" data-query="<?php echo $_GET['query'];?>">
		...
		<form id="searchForm" method="get">
			...
			<div class="col-sm-6 search-form">
				<div class="input-group">
					<input type="text" class="form-control input-sm" placeholder="Search by title, message" name="query" value="">
	...
	<div class="row box-body">
		...
		if($data['items']){
									...
									<?php 
										foreach($data['items'] as $key => $val){
											...
											<tr id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
												...
												<td data-link="name" class="item-open item-name '.($data['items'][$key]['status']==0?'unread':'').'"> '.$data['items'][$key]['email'].' <span>'.$data['items'][$key]['title'].'</span></td>
												<td class="time">'.date("Y-m-d H:i:s", $data['items'][$key]['time']).'</td>
											</tr>';
										}
									?>
									</tbody>
			...
			echo '
			<div class="col-md-12">
				Not found any mailbox here, be first and create it once!
			</div>';
		}
		?>
	</div>
			...
			if(isset($data['page'])){
				...
				<ul class="pagination">';
					for($i=1;$i<=$data['pages'];$i++){
						echo '
						<li><a href="/mailbox'.$urla.'/page/'.$i.''.$urlb.'" '.($_GET['page']==$i?'class="active"':'').'>'.$i.'</a></li>';
					}
					echo '
				</ul>';
			}
...
<script type="text/javascript">
	$(document).ready(function () {
		// Initialize actions
		once.mailbox.actions.mainInit($(this));
	});
</script>