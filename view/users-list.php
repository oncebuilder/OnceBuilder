...
?>
<div id="users-data" class="box" data-ajax="true" data-c="<?php echo $_GET['c'];?>" data-o="<?php echo $_GET['o'];?>" data-type_id="<?php echo $_GET['type_id'];?>" data-category_id="<?php echo $once->url_slug($_GET['option']);?>" data-sort_by="<?php echo $_GET['sort_by'];?>" data-ids="<?php echo $_GET['idsxs'];?>" data-page="<?php echo $_GET['page'];?>" data-query="<?php echo $_GET['query'];?>">
		...
		<form id="searchForm" method="get">
			...
			<div class="col-sm-6 search-form">
				<div class="input-group">
					<input type="text" class="form-control input-sm" placeholder="Search by username, email" name="query" value="">
	...
	<div class="row box-body">
								...
								<?php 
									foreach($data['items'] as $key => $val){
										...
										<tr id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
											...
											}else if($data['items'][$key]['type_id']==-1){
												echo 'Not activated';
											}else if($data['items'][$key]['type_id']==-2){
												echo 'Banned';
											}else if($data['items'][$key]['type_id']==0){
												echo 'User';
											}else{
												echo 'Unknown';
											}
											...
											if($data['items'][$key]['type_id']==-1){
												echo'<td data-link="status" class="item-status"><span class="label label-warning">Not activated</span></td>';
											}else if($data['items'][$key]['type_id']==-2){
												echo'<td data-link="status" class="item-status"><span class="label label-danger">Banned</span></td>';
											}else{
		...
		<?php 
			...
			echo '
			<div class="col-md-12">
				Not found any users here, be first and create it once!
			</div>';
		}
		?>
	</div>
