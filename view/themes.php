	...
	<form class="btn-group form-inline margin">
		<div class="form-group">';
			if($_GET['v']=='installed'){
				...
				<button class="btn btn-default btn-sm item-import" type="button"><i class="fa fa-cloud-upload"></i>&nbsp; import</button>';
			}else{
				$header.='
...
<div class="row">
	<div id="categories-data" class="col-md-'.($_GET['v']=='installed'?'3':'12 hidden').'">
		<ul class="list-group nav nav-pills nav-stacked">
			<li class="header">Themes categories
				...
				<li class="list-group-item current" data-id="0">
					<div class="list-group-header"><i class="fa fa-bars"></i><span>All themes</span></div>
				</li>';
	...
	<div id="content-body" class="col-md-'.($_GET['v']=='installed'?'9':'12').'">
		<script type="text/javascript">
			$(function() {
				$.get("view.php?c='.($_GET['route']==''?'themes':$_GET['route']).'&o='.($_GET['v']==''?'installed':$_GET['v']).'", function(data) {
					$("#content-body").html(data);
				})
				.error(function() { console.log("View Error: themes"); });
			});
		</script>
	</div>
</div><!-- /.row -->
<script src="js/once.themes.js" type="text/javascript"></script>';
$obj['html']=$str;
