...
if($_GET['option']!=''){
	...
	$stmt = $once->pdo->prepare("SELECT * FROM edit_plugins_categories WHERE LOWER(name) LIKE :category");
...
# GET DATA -------------------
//$categories=$once->category_get('plugins');
...
# GET REMOTE DATA -------------------
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://oncebuilder.com/once/ajax.php?c=plugins&o=api_search&type_id=".$_GET['type_id']."&category_id=".$_GET['category_id']."&sort_by=".$_GET['sort_by']."&page=".$_GET['page']."&ids=".$_GET['ids']."&query=".$_GET['query']."");
	...
	<div class="row box-body">
		...
		if(isset($data['items'])){
			foreach($data['items'] as $key => $val){
				...
				<div class="col-md-6">
					<div class="plugin-item">
						...
						<div class="row plugin-footer" id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
							<div class="col-md-12">';
								if($data['items'][$key]['price']==0){
									echo '<button type="button" class="btn btn-success btn-sm pull-right item-download"><i class="fa fa-plus"></i>&nbsp;&nbsp;&nbsp; Download</button>';
								}
								echo'
			...
			if(isset($response['error'])){
				...
				echo '
				<div class="col-md-12">
					Not found any plugins here, be first and create it once!
				</div>';
			}
		}
		?>
			...
			if(isset($data['page'])){
				...
				<ul class="pagination">';
					for($i=1;$i<=$data['pages'];$i++){
						echo '
						<li><a href="/plugins'.$urla.'/page/'.$i.''.$urlb.'" '.($_GET['page']==$i?'class="active"':'').'>'.$i.'</a></li>';
					}
					echo '
				</ul>';
			}
...
<script type="text/javascript">
	$(document).ready(function () {
		// Initialize actions
		once.plugins.actions.mainInit($(this));
	});
</script>