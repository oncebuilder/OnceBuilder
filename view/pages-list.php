...
if($_GET['option']!=''){
	...
	$stmt = $once->pdo->prepare("SELECT * FROM edit_pages_categories WHERE LOWER(name) LIKE :category");
...
$once->set_data(array(
	...
	"query_in" => array('name','title'),
	"where" => ''
));

# GET DATA -------------------
$data=$once->once_select_items_page('pages','project_id');

# GET DATA -------------------
$layers=$once->once_select_items('layers');
if(isset($layers['items'])){
	foreach($layers['items'] as $key => $val){
		$layers_a[$layers['items'][$key]['id']]=$layers['items'][$key]['name'];
	}
}

# GET DATA -------------------
$routes=$once->once_select_items('routes');
if(isset($routes['items'])){
	foreach($routes['items'] as $key => $val){
		$routes_a[$routes['items'][$key]['page_id']]=$routes['items'][$key]['name']."".$routes['items'][$key]['name_id'];
	}
}
...
?>
<div id="pages-data" class="box" data-ajax="true" data-c="<?php echo $_GET['c'];?>" data-o="<?php echo $_GET['o'];?>" data-type_id="<?php echo $_GET['type_id'];?>" data-category_id="<?php echo $once->url_slug($_GET['option']);?>" data-sort_by="<?php echo $_GET['sort_by'];?>" data-ids="<?php echo $_GET['idsxs'];?>" data-page="<?php echo $_GET['page'];?>" data-query="<?php echo $_GET['query'];?>">
	...
	<div class="row box-body">
		...
		<?php
			...
			echo '
			<div class="col-md-12">
				Not found any pages here, be first and create it once!
			</div>';
		}
		?>
	</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		// Initialize actions
		once.pages.actions.mainInit($(this));
	});
</script>