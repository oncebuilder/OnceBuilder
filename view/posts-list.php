...
if($_GET['option']!=''){
	...
	$stmt = $once->pdo->prepare("SELECT * FROM edit_posts_categories WHERE LOWER(name) LIKE :category");
...
$once->set_data(array(
	...
	"query_in" => array('title','description'),
	"where" => ''
));

# GET DATA -------------------
$data=$once->once_select_items_page('posts');
?>
<div id="posts-data" class="box" data-ajax="true" data-c="<?php echo $_GET['c'];?>" data-o="<?php echo $_GET['o'];?>" data-type_id="<?php echo $_GET['type_id'];?>" data-category_id="<?php echo $once->url_slug($_GET['option']);?>" data-sort_by="<?php echo $_GET['sort_by'];?>" data-ids="<?php echo $_GET['idsxs'];?>" data-page="<?php echo $_GET['page'];?>" data-query="<?php echo $_GET['query'];?>">
		...
		<form id="searchForm" method="get">
			...
			<div class="col-sm-6 search-form">
				<div class="input-group">
					<input type="text" class="form-control input-sm" placeholder="Search by title, description" name="query" value="">
	...
	<div class="row box-body">
		...
		if($data['items']){
									...
									<?php 
										foreach($data['items'] as $key => $val){
											...
											<tr id="item_'.$data['items'][$key]['id'].'" data-id="'.$data['items'][$key]['id'].'">
												...
												<td data-link="title" class="item-title">'.$data['items'][$key]['title'].'</td>
												<td data-link="status">
													'.($data['items'][$key]['type_id']==1?'<span class="label label-success">Published</span>':'').'
													'.($data['items'][$key]['type_id']==2?'<span class="label label-info">Draft</span>':'').'
													'.($data['items'][$key]['type_id']==3?'<span class="label label-primary">Pedding</span>':'').'
													'.($data['items'][$key]['type_id']==5?'<span class="label label-default">Junk</span>':'').'
												</td>
												<td data-link="date" class="item-date">15:10 22.07.2015</td>
												<td>
													<a class="item-edit" title="post edit" style="cursor: pointer;"><i class="fa fa fa-edit"></i></a>
														&nbsp;&nbsp;
													<a class="item-delete" title="post delete" style="cursor: pointer;"><i class="fa fa-trash-o"></i></a>
												</td>
											</tr>';
										}
									?>
			...
			echo '
			<div class="col-md-12">
				Not found any posts here, be first and create it once!
			</div>';
		}
		?>
	</div>
			...
			if(isset($data['page'])){
				...
				<ul class="pagination">';
					for($i=1;$i<=$data['pages'];$i++){
						echo '
						<li><a href="/posts'.$urla.'/page/'.$i.''.$urlb.'" '.($_GET['page']==$i?'class="active"':'').'>'.$i.'</a></li>';
					}
					echo '
				</ul>';
			}
...
<script type="text/javascript">
	$(document).ready(function () {
		// Initialize actions
		once.posts.actions.mainInit($(this));
	});
</script>