...
$content = $connection->get('application/rate_limit_status.json?resources=help,users,search,statuses');
print_r($content);
...
/* statuses/show */
$method = "statuses/show.json?id={$status->id_str}";
twitteroauth_row($method, $connection->get($method), $connection->http_code);

/* statuses/destroy */
$statusx = $connection->post("statuses/destroy/{$status->id_str}.json", $parameters);
twitteroauth_row("statuses/destroy/{$status->id_str}.json", $statusx, $connection->http_code, $parameters);
...
/* statuses/destroy */
$method = "statuses/destroy/{$status->i}.json";
