 ...
 *
 *	This is OnceBuilder Tutorials plugin (once.tutorials)
 *
*/
...
once.tutorials = {
	...
	bulkAction: function(obj){
		...
		// Set action o
		$("#checkForm").attr("action",once.path+"/ajax.php?c=tutorials&o=bulk_action");
		// Submit form
		$("#checkForm").submit();
	},
	sortAction: function(obj){
		if($("#tutorials-data").data("ajax")){
			$("#sort-label").html(obj.html());
			// Refresh items list by sort key
			$.get(once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o")+"&type_id="+$("#tutorials-data").data("type_id")+"&category_id="+$("#tutorials-data").data("category_id")+"&sort_by="+obj.data("sort")+"&page="+$("#tutorials-data").data("page")+"&ids="+$("#tutorials-data").data("ids")+"&query="+$("#tutorials-data").data("query"), function(data) {
			...
			// Refresh items list by sort key
			$("#sort-label").html(obj.html());
			document.location.href='/tutorials?category_name='+$("#tutorials-data").data('category')+'&p='+$("#tutorials-data").data("page")+'&sort_by='+obj.data("sort")+"&query="+$("#tutorials-data").data('query');
		}
	},
	displayLimit: function(obj){
		// Call for set limit
		$.getJSON(once.path+"/ajax.php?c=tutorials&o=set_limit&limit="+$(obj).val(), function(data) {
	...
	openPage: function(obj){
		...
		$.get(once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o")+"&type_id="+$("#tutorials-data").data("type_id")+"&category_id="+$("#tutorials-data").data("category_id")+"&sort_by="+$("#tutorials-data").data("sort_by")+"&page="+obj.html()+"&ids="+$("#tutorials-data").data("ids")+"&query="+$("#tutorials-data").data("query"), function(data) {
			$("#content-body").html(data);
		})
		.error(function() { console.log("Request Error: "+$("#tutorials-data").data("o")); });
	},
	// View function
	itemNew: function(obj){
		...
		$.ajax({
			type: 'POST',
			url: once.path+"/ajax.php?c=tutorials&o=item_new&type_id="+$("#tutorials-data").data('type_id')+"&category_id="+$("#tutorials-data").data('category_id'),
			success: function(data) { 
				// Refresh items list if response ok
				if(data.status=='ok'){
					// Call for refresh
					$.get(once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o")+"&type_id="+$("#tutorials-data").data("type_id")+"&category_id="+$("#tutorials-data").data("category_id"), function(data) {
						$("#content-body").html(data);
					})
	...
	itemStar: function(obj){
		...
		var row=col.parent();
		// Call for star item
		$.getJSON(once.path+"/ajax.php?c=tutorials&o=item_star&id="+row.data("id"), function(data) {
			// Refresh items list if response ok
			if(data.status=='ok'){
	...
	itemDelete: function(obj){
		...
		if(r){
			...
			$.ajax({
				type: 'POST',
				url: once.path+"/ajax.php?c=tutorials&o=item_delete&id="+id,
				success: function(data) { 
					// Refresh items list if response ok
					if(data.status=='ok'){
						...
						$("#item_"+id).remove();
						// Call for refresh
						$.get(once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o")+"&ids="+$("#tutorials-data").data("ids")+"&type_id="+$("#tutorials-data").data("type_id")+"&category_id="+$("#tutorials-data").data("category_id")+"&sort_by="+$("#tutorials-data").data("sort_by")+"&page="+$("#tutorials-data").data("page")+"&query="+$("#tutorials-data").data("query"), function(data) {
							$("#content-body").html(data);
						})
						.error(function() { console.log("Request Error: "+$("#tutorials-data").data("o")); });
					}else{
	...
	itemEditPublish: function(obj){
		...
		if(r){
			...
			$.ajax({
				type: 'POST',
				url: once.path+"/ajax.php?c=tutorials&o=item_publish&id="+id,
				success: function(data) { 
					// Refresh items list if response ok
					if(data.status=='ok'){
						console.log(data);
		...
		if(r){
			...
			$.ajax({
				...
				success: function(data) { 
					// Refresh items list if response ok
					if(data.status=='ok'){
						$("#tutorial-data .item-close").click();
						// Intermediatly delete from dom then refresh items list
						$("#item_"+id).remove();
						// Call for refresh
						$.get(once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o")+"&ids="+$("#tutorials-data").data("ids")+"&type_id="+$("#tutorials-data").data("type_id")+"&category_id="+$("#tutorials-data").data("category_id")+"&sort_by="+$("#tutorials-data").data("sort_by")+"&page="+$("#tutorials-data").data("page")+"&query="+$("#tutorials-data").data("query"), function(data) {
	...
	itemEditSave: function(obj){
		...
		if($("#tutorial-data").data("source")){
			once.tutorials.itemEditSource();
		}else if($("#tutorial-data").data("content")){
			once.tutorials.itemEditContent();
		}else{
			$("#editForm").submit();
		}
	},
	itemEditContent: function(obj){
		...
		$.post(once.path+"/ajax.php?c=tutorials&o=save_content&id="+$("#tutorial-data").data("id"), { source: source }, function(data) {
	...
	itemEditPreviewRaw: function(obj){
		var id=$("#tutorial-data").data('id');
		window.open(once.path+'/tutorials/'+id+'/tutorial.html','_blank');
	},
	itemEditApprove : function(obj){
		...
		if(r){
			...
			$.ajax({
				type: 'POST',
				url: once.path+"/ajax.php?c=tutorials&o=item_approve&id="+id,
				success: function(data) { 
					// Refresh items list if response ok
					if(data.status=='ok'){
						$("#tutorial-data .item-close").click();
	...
	itemEditInsightsImage: function(obj){
		...
		$.getJSON(once.path+"/ajax.php?c=tutorials&o=item_insights_image&id="+id, function(data) {
			// Refresh items list if response ok
			if(data.status=='ok'){
				$("#item-logo").attr("src","/admin/tutorials/"+data.item.id+"/logo.jpg?"+Math.random());
				console.log(data);
			}else{
				console.log("Action Error: "+data.error);
			}
	...
	itemUserPublish: function(obj){
		...
		if(r){
			...
			$.ajax({
				type: 'POST',
				url: once.path+"/ajax.php?c=tutorials&o=item_user_publish&id="+id,
				success: function(data) { 
					// Refresh items list if response ok
					if(data.status=='ok'){
						document.location.href="/tutorial/"+id;
						console.log(data);;
					}else{
						console.log("Action Error: "+data.error);
					}
	...
	itemUserFork: function(obj){
		...
		var id=$("#tutorial-data").data('id');
		// Send ajax request to api
		$.getJSON(once.path+"/ajax.php?c=tutorials&o=item_user_fork&id="+id, function(data) {
			// Refresh items list if response ok
			if(data.status=='ok'){
				document.location.href='/tutorial/'+data.item.id;
				console.log(data);
			}else{
				console.log("Action Error: "+data.error);
			}
	...
	itemUserVote: function(obj){
		...
		var id=$("#tutorial-data").data('id');
		// Send ajax request to api
		$.getJSON(once.path+"/ajax.php?c=tutorials&o=item_user_vote&id="+id, function(data) {
			// Refresh items list if response ok
			if(data.status=='ok'){
				alert('Thanks. voted!');
				console.log(data);
	...
	itemUserDownload: function(obj){
		...
		document.location.href="/ajax.php?c=tutorials&o=item_user_download&id="+id;
	},
	itemUserReport: function(obj){
		// Read and open report dialog
		$("#item-reported").load(once.path+"/dialog.php?c=tutorials&o=report&id="+obj.parent().parent().data('id'), function() {
	...
	mainInit: function(obj){
		...
		if($('input[type="checkbox"]').length){
			...
			$(".bulk-action").click(function () {
				once.tutorials.bulkAction($(this));
			});
		}
		
		// Sort actions
		$(".sort-action").click(function () {
			once.tutorials.sortAction($(this));
		});
		
		// Star item
		$(".item-star").click(function () {
			once.tutorials.itemStar($(this));
		});

		// Initialize itemEdit dialog
		once.tutorials.dialogs.itemEdit(".item-edit");
		
		// Delete item
		$(".item-delete").click(function () {
			once.tutorials.itemDelete($(this));
		});
		
		// Set display limit
		$(".display-limit").change(function () {
			once.tutorials.displayLimit($(this));
		});
		...
		$(".pagination a").click(function (e) {
			e.preventDefault();
			e.stopPropagation();
			once.tutorials.openPage($(this));
		});
		
		// Initialize checkForm
		once.tutorials.forms.checkForm($(this));
				
		// Initialize searchForm
		once.tutorials.forms.searchForm($(this));

		// Initialize / sandbox
		once.tutorials.initialized();
	},
	editInit: function(obj){
		// After show selected tab
		$('#tutorial-data a[data-toggle="tab"]').on('show.bs.tab', function (e) {
			...
			if(tab.attr('href')=='#edit_settings'){
				...
				$("#tutorial-data").data("source",false);
				$("#tutorial-data").data("content",false);
				...
				$("#tutorial-data").data("source",false);
				$("#tutorial-data").data("content",true);
			}else if(tab.attr('href')=='#edit_source'){
				// Setting data to make it work with keypress
				$("#tutorial-data").data("source",true);
				$("#tutorial-data").data("content",false);
				...
				// Set other settings
				$("#tutorial-data").data("editor",tab.attr('data-editor'));
				$("#tutorial-data").data("path",tab.attr('data-path'));
				$("#tutorial-data").data("file",tab.attr('data-file'));
				$("#tutorial-data").data("title",tab.html());
		...
		$(window).keypress(function(event) {
			...
			if(event.ctrlKey && event.which==115){
				...
				once.tutorials.itemEditSource($(this));
			}
		});
		
		// Approve item
		$("#tutorial-data .item-insights-image").click(function () {
			once.tutorials.itemEditInsightsImage($(this));
		});
		
		// Publish item
		$("#tutorial-data .item-publish").click(function () {
			once.tutorials.itemEditPublish($(this));
		});
		
		// Delete item
		$("#tutorial-data .item-delete").click(function () {
			once.tutorials.itemEditDelete($(this));
		});
		
		// Approve item
		$("#tutorial-data .item-approve").click(function () {
			once.tutorials.itemEditApprove($(this));
		});
		
		// Save item
		$("#tutorial-data .item-save").click(function () {
			once.tutorials.itemEditSave($(this));
		});

		// Save item
		$("#tutorial-data .item-presave").click(function () {
			// item save & source and change location
			$("#editForm").submit();
		});
		
		// Change logo
		$("#tutorial-data .item-logo").click(function () {
			once.tutorials.itemEditLogo($(this));
		});
		
		// Preview raw file
		$("#tutorial-data .item-preview-raw").click(function () {
			once.tutorials.itemEditPreviewRaw($(this));
		});
		
		// Initialize report dialog
		once.tutorials.dialogs.itemReport("#tutorial-data .item-report");

		// Initialize editForm
		once.tutorials.forms.editForm($(this));
		
		// Initialize uploadImage
		once.tutorials.forms.uploadImage($(this));
		...
		// User save
		$("#tutorial-data .item-user-save").click(function () {
			$("#editForm").submit();
		});
		
		// User publish
		$("#tutorial-data .item-user-publish").click(function () {
			once.tutorials.itemUserPublish($(this));
		});
		
		// User vote
		$("#tutorial-data .item-user-vote").click(function () {
			once.tutorials.itemUserVote($(this));
		});
		
		// User download
		$("#tutorial-data .item-user-download").click(function () {
			once.tutorials.itemUserDownload($(this));
		});
		
		// User fork
		$("#tutorial-data .item-user-fork").click(function () {
			once.tutorials.itemUserFork($(this));
		});
	},
}

once.tutorials.dialogs = {
	itemReport: function(obj){
		...
		// Read and open report dialog
		$(obj).click(function () {
			$("#item-report").load(once.path+"/dialog.php?c=tutorials&o=report&id="+$("#tutorial-data").data("id"), function() {
	...
	itemPublish: function(obj){
		...
		$(obj).click(function () {
			$("#item-publish").load(once.path+"/dialog.php?c=tutorials&o=publish", function() {
	...
	itemImport: function(obj){
		...
		$(obj).click(function () {
			$("#item-import").load(once.path+"/dialog.php?c=tutorials&o=import", function() {
	...
	itemEdit: function(obj){
		...
		$(obj).click(function () {
			$("#item-edit").load(once.path+"/dialog.php?c=tutorials&o=edit&id="+$(this).parent().parent().data("id"), function() {
...
once.tutorials.forms = {
	checkForm: function(obj){
		$("#checkForm").attr("action",once.path+"/view.php?c=tutorials");
		var options = {
			dataType:  "json",
			success: function(data){
				...
				if($("#checkForm").data("type")=='stared' && $("#checkForm").data("module")=='unstar'){
					for(var i=0;i<data.tutorials.length;i++){
						console.log(data.tutorials[i]);
						$('tr[data-id="'+data.tutorials[i]+'"]').remove();
					}
				}else if($("#checkForm").data("module")=='star'){
					for(var i=0;i<data.tutorials.length;i++){
						var obj = $("tr[data-id='"+data.tutorials[i]+"'] .page-star");
						var glyph = $(obj).hasClass("glyphicon");
						var fa = $(obj).hasClass("fa");
						//Switch states
						if (glyph) {
				...
				$.get(once.path+"/view.php?c=tutorials&o=installed&t="+$("#checkForm").data("type")+"&p="+$(".pagination").data("p"), function(data) {
					$("#content-body").html(data);
				})
	...
	searchForm: function(obj){
		$("#searchForm").attr("action",once.path+"/view.php?c=tutorials&o="+$("#tutorials-data").data("o"));
		var options = {
	...
	editForm: function(obj){
		$("#editForm").attr("action",once.path+"/ajax.php?c=tutorials&o=item_edit&id="+$("#tutorial-data").data("id"));
		var options = {
			dataType:  "json",
			success: function(data){
				// If response ok 
				if(data.status=='ok'){
					...
					if($("#tutorial-data").data('redirect')!=undefined){
						// Get source
						var source = rawurlencode($(".textarea").val());
						$.post(once.path+"/ajax.php?c=tutorials&o=save_content&id="+$("#tutorial-data").data("id"), { source: source }, function(data) {
							document.location.href=$("#tutorial-data").data('redirect');
						})
						.error(function() { console.log("Request Error: save_content"); });
					}
	...
	reportForm: function(obj){
		$("#reportForm").attr("action",once.path+"/ajax.php?c=tutorials&o=item_user_report&id="+$("#tutorial-data").data("id"));
		var options = {
			dataType:  "json",
			success: function(data){
				// If response ok 
				if(data.status=='ok'){
					$("#reportForm label[for='message']").html("Tutorial reported!");
					$("#reportForm textarea").attr("disabled",true);
					
					console.log("Tutorial reported!");
				}else{
					if(data.errors[0]=='user not logged'){
						document.location.href="/login";
					}else{
	...
	uploadImage: function(obj){
		...
		$("#uploadImage").attr("action",once.path+"/ajax.php?c=tutorials&o=upload_image&id="+$("#tutorial-data").data("id"));
		var options = { 
			dataType:  "json",
			success: function(data){
...
$(document).ready(function () {
	...
	$(".item-new").click(function () {
		once.tutorials.itemNew($(this));
    });
	
	// Initialize publish dialog
	once.tutorials.dialogs.itemPublish(".item-publish");
	
	// Initialize import dialog
	once.tutorials.dialogs.itemImport(".item-import");
	...
	if($("#tutorial-data").data("ajax") || $("#tutorials-data").data("ajax")){
		once.loadJSfile(once.path+'/js/jquery.form.js');
		//once.loadJSfile('//oss.maxcdn.com/jquery.form/3.50/jquery.form.min.js');
	}
	
	// Initialize tutorials actions
	if($("#tutorials-data").length>0){
		// Sort actions
		$("#tutorials-data .sort-action").click(function () {
			once.tutorials.sortAction($(this));
		});
	...
	if($("#tutorial-data").length){
		...
		once.tutorials.actions.editInit();
	}
